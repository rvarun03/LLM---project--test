...
async function startRecording(config, client) {
  ...
  try {
    ...
    function isNoiseRequest(url, resourceType, method, status) {
      if (!url) return true;
      ...
      if (['image', 'font', 'stylesheet', 'media', 'texttrack', 'eventsource', 'websocket', 'manifest', 'other'].includes(lowerType)) {
        return true;
      }

      if (/\.(?:png|jpe?g|gif|svg|webp|ico|bmp|woff2?|ttf|eot|otf|css|mp[34]|webm|ogg|wav|avif|cur|map|wasm|pdf|zip|txt)(?:[?#].*)?$/i.test(lowerUrl)) {
        return true;
      }

      // Static script bundles and library chunks (unless explicit backend API route that returns JSON)
      if (lowerType === 'script' || (/\.(?:js|mjs|cjs)(?:[?#].*)?$/i.test(lowerUrl) && !lowerUrl.includes('/api/'))) {
        return true;
      }
    ...
    function attachPageListeners(page) {
      ...
      page.on('request', (request) => {
        ...
        if (isNoiseRequest(reqUrl, request.resourceType(), request.method())) return;
      ...
      page.on('response', async (response) => {
        ...
        if (isNoiseRequest(reqUrl, request.resourceType(), request.method(), status)) return;
